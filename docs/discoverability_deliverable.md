...
|----------|---------------------|-------------|
| Cursor users | system prompt, .cursorrules | `coding-assistant` template + 60s install block |
...
| `pyproject.toml` | Expanded keywords, discoverability doc URL | PyPI/search indexing |
...
- **Cursor:** `ri-engine improve --template coding-assistant` → paste into `.cursor/rules`
...
`prompt-engineering`, `llm`, `system-prompts`, `cursor`, `cli`, `open-source`, `prompt-improvement`, `chatgpt`, `agents`
